...
// ?? Palette ???????????????????????????????????????????????????????????????????
const EMOTION_META = {
  depression: { label: "?곗슱 吏??, color: "#00F5FF", glow: "#00F5FFCC", icon: "?? },
  anxiety: { label: "遺덉븞 吏??, color: "#FF6B00", glow: "#FF6B00CC", icon: "燧? },
  anger: { label: "遺꾨끂 吏??, color: "#FF0055", glow: "#FF0055CC", icon: "?? },
  joy: { label: "湲곗겏 吏??, color: "#39FF14", glow: "#39FF14CC", icon: "?? },
  stability: { label: "?덉젙 吏??, color: "#FFE600", glow: "#FFE600CC", icon: "?? },
  discomfort: { label: "遺덉풄 吏??, color: "#FF3DFF", glow: "#FF3DFFCC", icon: "?? },
};
...
export default function MusicVirus() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        setLoading(true);
        setLoadingStatus("?뱻 FETCHING LAST.FM GLOBAL CHART...");

        // 1. Last.fm 湲濡쒕쾶 Top 50 李⑦듃
        const chartRes = await fetch(
          `${LASTFM_BASE}/?method=chart.getTopTracks&api_key=${LASTFM_API_KEY}&format=json&limit=50`
        );
        if (!chartRes.ok) throw new Error("Last.fm ?붿껌 ?ㅽ뙣");

        const chartData = await chartRes.json();
        const rawTracks = chartData?.tracks?.track || [];
        if (rawTracks.length === 0) throw new Error("?몃옓 ?놁쓬");

        setLoadingStatus(`?렦 ANALYZING ${rawTracks.length} TRACKS...`);

        // 2. 媛??몃옓 蹂묐젹 泥섎━: Last.fm ?쒓렇 + iTunes 誘몃━?ｊ린
        // ?깅뒫???꾪빐 諛곗튂 泥섎━ (10媛쒖뵫)
        const BATCH = 10;
        let allItems = [];
        ...
        for (let b = 0; b < rawTracks.length; b += BATCH) {
          ...
          setLoadingStatus(`?뵇 LOADING ${b + 1}??{Math.min(b + BATCH, rawTracks.length)} / ${rawTracks.length}...`);
        ...
        console.error("API ?ㅻ쪟, 紐⑦겕 ?곗씠???ъ슜:", err);
      ...
      <header style={{
        ...
        <div>
          <div style={{ fontSize: isMobile ? 8 : 9, letterSpacing: isMobile ? "0.15em" : "0.3em", color: "#F0C060AA", marginBottom: 4, textTransform: "uppercase" }}>
            ??LIVE 쨌 Social Emotional Contagion
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{
            ...
            {isMobile ? `${tracks.length} tracks` : `${tracks.length} tracks 쨌 Last.fm Global`}
          </div>
          {/* iTunes 誘몃━?ｊ린 踰꾪듉 */}
          {selected.previewUrl && (
          ...
          <div style={{ opacity: animating ? 0 : 1, transition: "opacity 0.28s", position: "relative", zIndex: 2 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{
                  ...
                  {[
                    ...
                    { label: "KEY", val: selected.mode === "minor" ? "?⑥“" : "?μ“" },
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: "#F0E8D4" }}>
                  {EMOTION_META[dominantEmotion].label} 쨌 媛뺣룄 {Math.round(scores[dominantEmotion] * 100)}
                </div>
                <div style={{ fontSize: 10, color: "#5A5A4A", marginTop: 3 }}>
                  ?꾪뙆 吏??{Math.round(scores.contagion * 100)}% 쨌 {selected.streams >= 1e9 ? `${(selected.streams / 1e9).toFixed(1)}B` : `${(selected.streams / 1e6).toFixed(0)}M`} plays
                  {selected.listeners ? ` 쨌 ${(selected.listeners / 1e6).toFixed(1)}M listeners` : ""}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(195px, 1fr))", gap: 12, marginBottom: 18 }}>
              ...
              <div style={{ background: "#0E1020A0", border: "1px solid #F0C06020", borderRadius: 14, padding: "15px 17px", backdropFilter: "blur(4px)" }}>
                <div style={{ fontSize: 9, letterSpacing: "0.12em", color: "#F0C060", marginBottom: 10, textTransform: "uppercase" }}>??Tempo 쨌 Mode</div>
                ...
                <div style={{ marginTop: 8 }}>
                  <div style={{
                    ...
                    {selected.mode === "minor" ? "?⑥“ (Minor)" : "?μ“ (Major)"}
                  </div>
                </div>
                <div style={{ marginTop: 10, fontSize: 10, color: "#4A4A3A", lineHeight: 1.7 }}>
                  ?⑥“ + 鍮좊Ⅸ BPM ??遺덉븞媛?利앺룺<br />?⑥“ + ?먮┛ BPM ???곗슱媛??좊컻
                </div>
              </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 9, letterSpacing: "0.2em", color: "#3A3A2A", marginBottom: 12, textTransform: "uppercase" }}>
                ?ы쉶 吏묐떒 媛먯젙 ?됯퇏 쨌 {tracks.length}怨?              </div>
